...
def main(config: DictConfig):
    ...
    examination = EmbeddingParameter(
        EmbeddingParameterConfig(use_feature="positions", parameters=22),
        rngs=rngs,
    )

    attraction = DeepCrossParameter(
        DeepCrossParameterConfig(use_feature="query_doc_features", features=768),
        rngs=rngs,
    )
    ...
    model = MixtureModel(
        models=[
            ...
            DocumentCTRModel(ctr=attraction, rngs=rngs),
            PositionBasedModel(
                examination=examination, attraction=attraction, rngs=rngs
            ),
        ],
    )
    ...
    test_rel_df = trainer.test_ranking(model, test_relevance_loader)
